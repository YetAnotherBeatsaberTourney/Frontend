...
ws.onmessage = async function(event) {
    ...
    if (jsonObj.Type == 6) {
        if (jsonObj.command == "updateSpectator") {
            ...
            document.getElementById("ACCGoalTextSpan").style.opacity = "0";
            setTimeout(function() {
                document.getElementById("CurrentPlayer").innerHTML = jsonObj.Player;
                document.getElementById("ACCGoalTextSpan").innerHTML = jsonObj.ACCGoal+"%";
                document.getElementById("CurrentPlayer").style.opacity = "1";
                document.getElementById("ACCGoalTextSpan").style.opacity = "1";
            }, 500);
        }
        if (jsonObj.command == "resetSpectator") {
            document.getElementById("CurrentPlayer").style.opacity = "0";
            document.getElementById("ACCGoalTextSpan").style.opacity = "0";
            setTimeout(function() {
                document.getElementById("CurrentPlayer").innerHTML = "None";
                document.getElementById("ACCGoalTextSpan").innerHTML = "0%";
            }, 500);
        }
